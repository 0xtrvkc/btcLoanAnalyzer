...
test('collateral repayment renders recovery, partial deployment and unavailable states',()=>{
 ...
 assert.match(app.nodes.get('repay-recovery-price').textContent,/\$/);assert.match(app.nodes.get('repay-recovery-note').textContent,/No finite BTC quantity break-even/);
 ...
 assert.equal(app.nodes.get('repay-wallet').textContent,'Unavailable');assert.doesNotMatch(app.nodes.get('repay-wallet-flow').innerHTML,/NaN|Infinity/);
});

test('current-price summary and glance chart separate holding, loan, and combined results',()=>{
 const app=harness(600);app.run('runHiddenCommand()');app.run('render()');
 assert.match(app.nodes.get('repay-be-title').textContent,/At current BTC/);
 assert.match(app.nodes.get('repay-be-hold').innerHTML,/Holding P\/L vs\. cost basis/);
 ...
 assert.match(app.nodes.get('repay-wallet-flow').innerHTML,/\$[\d,]+\.\d{2} now/);
 assert.match(app.nodes.get('wallet-compose-track').innerHTML,/compose-benchmark/);
 assert.match(app.nodes.get('wallet-compose-total').textContent,/BTC · \$/);
